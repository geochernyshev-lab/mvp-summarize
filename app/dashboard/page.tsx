...
type SummaryRow={id:string;created_at:string;file_name:string;file_pages:number;file_bytes:number;summary:string};
...
export default function Dashboard(){
  ...
  async function loadData(){
    ...
    try{
      ...
      const { data:rows } = await supabase.from('summaries').select('id,created_at,file_name,file_pages,file_bytes,summary').order('created_at',{ascending:false}).limit(20);
      setSummaries((rows as any)??[]);
    } finally { setLoading(false); }
  }

  useEffect(()=>{ loadData(); const h=()=>loadData(); window.addEventListener('summary-added',h); return ()=>window.removeEventListener('summary-added',h); },[]);
    ...
    return (
      <div className="grid">
        ...
        <div className="muted" style={{marginTop:6}}>
          <a className="btn ghost" href="/login">Вход</a>&nbsp;<a className="btn primary" href="/signup">Регистрация</a>
        </div>
      </div>
    );
  }
              ...
              <article className="summary-card" key={row.id}>
                <div className="summary-meta">
                  ...
                  <div className="meta-sub">
                    <span>{new Date(row.created_at).toLocaleString()}</span><span>•</span>
                    <span>{row.file_pages} стр.</span><span>•</span>
                    <span>{(row.file_bytes/1024).toFixed(1)} KB</span>
                  </div>
                </div>

                <div className={`summary-body ${isOpen ? '' : 'clamped'}`}>
                  {row.summary}
                </div>

                <div className="summary-actions">
                  <button className="btn ghost" onClick={()=>toggle(row.id)}>{isOpen?'Скрыть':'Показать полностью'}</button>
